...
describe('registry pipeline — valid repository', () => {
  ...
  it('builds nine tools, nine presets and the three visible categories', () => {
    ...
    expect(Object.keys(registry?.presets ?? {}).length).toBe(9);
  ...
  it('hides categories with fewer tools than navigation.minToolsPerCategory', async () => {
    ...
    const visibleAt = (min: number) =>
      buildCategories(taxonomy, registry.tools, {
        ...
        .filter((c) => c.visible)
        .map((c) => c.id);
    // logistics, business and developer have three tools each.
